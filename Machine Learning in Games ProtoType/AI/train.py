...
def train_simple_model(training_data_X, training_data_Y, restore_checkpoint_path, starting_step):
    ...
    with tf.name_scope("train"):
        ...
        with tf.Session() as sess:
            ...
            for step in range(number_of_epochs):
                ...
                #print(weights.shape)
                print(weights)
                #summary_test = tf.summary.histogram("layer_1/fully_connected/weights", weights)
                #writer.add_summary(summary_test, final_step)

                bias=sess.run(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'layer_1/fully_connected/bias'))
                print(bias)

                print('Epoch {} - Loss {} - Accuracy {}'.format(final_step, loss, current_accuracy))
    ...
    print('Completed')
    return checkpoint_file_path, final_step